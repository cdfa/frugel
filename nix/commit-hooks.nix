...
with pkgs;
{
  ...
  floskellConfigChangeHook = {
    ...
    entry = "${bash}/bin/bash -c 'shopt -s globstar; ${floskell}/bin/floskell $(${coreutils}/bin/ls {prelude,app,src,test,scout-src}/**/*.hs | ${gnused}/bin/sed \"/app\\/\\(Control\\/Timeout\\|Language\\/Javascript\\/JSaddle\\/Warp\\/Extra\\).hs/d\")'";
    files = "floskell.json$";
